...
{
    /// <summary>
    /// Synchronous <c>data:</c> URI decoding shared by every layer that needs to inspect a data URI's
    /// declared MIME type/bytes without going through the async
    /// <see cref="Network.DataUriNetworkLoader"/>/<see cref="Handlers.ImageLoadHandler"/> pipeline (e.g.
    /// to reject an unsupported MIME type before ever attempting to load it).
    /// </summary>
    internal static class DataUriUtils
    {
        public static bool TryDecodeDataUri(string? uri, out string mimeType, out byte[] bytes)
        {
            ...
            try
            {
                ...
                // with them percent-escaped (%2F, %2B, %3D), and Convert.FromBase64String throws on a
                // literal "%" in its input, so skipping this step here silently fails to decode such data.
                bytes = isBase64
